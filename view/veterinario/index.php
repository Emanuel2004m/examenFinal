...
<body>
                ...
                <table id="myTabla" class="table table-striped mb-0">
                    ...
                    <tbody>
                        <?php foreach ((array) $rows as $row) { ?>
                            <tr>
                                <td><?= $row['idVeterinario'] ?></td>
                                <td><?= $row['nomVeterinario'] ?></td>
                                <td><?= $row['cni'] ?></td>
                                <td><?= $row['telefono'] ?></td>
                                <td>
                                    <a class="btn btn-info" data-bs-toggle="modal" data-bs-target="#idver<?= $row['idVeterinario'] ?>">Ver</a>
                                    <a href="edit.php?id=<?= $row['idVeterinario'] ?>" class="btn btn-warning">Editar</a>
                                    <a class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#iddel<?= $row['idVeterinario'] ?>">Eliminar</a>
    ...
    <div class="container" id="ventana" style="display:none;">
        <div class="mb-3">
            <h2 style="font-size: 3.00rem;">Listado de Veterinarios</h2>
        </div>
        <div class="table-responsive table-scroll" data-mdb-perfect-scrollbar="true" style="position: relative; height:700px;">
                ...
                <tbody>
                    <?php foreach ($rows as $row) { ?>
                        <tr>
                            ...
                            <td colspan="3"><?= $row['nomVeterinario'] ?></td>
